...
import de.tk.dependencyanalyse.rapui.visgraph.internal.ThreeForceGraphJsBridge;
...
/**
 * RAP-BrowserWidget that hosts a 3D Force-Directed Graph renderer.
 *
 * <p>Public-API twin of {@link Neo4jNvlViewer} — same method names, same
 * semantics — but the underlying bridge is {@link ThreeForceGraphJsBridge}
 * and the HTML payload is the 3D viewer
 * ({@code /static/three/three-force-viewer.html}). Layout, physics, and
 * hierarchical-direction setters map to the 3D renderer's option names;
 * some vis-network-specific knobs (solver, stabilization iterations) are
 * best-effort no-ops at the bridge level.</p>
 *
 * <p>Requires WebGL — the JS viewer probes the canvas context on boot
 * and shows an inline error message when WebGL is unavailable. The
 * Java-side viewer still constructs normally in that case; subsequent
 * operations simply queue against a viewer that never signals
 * {@code tfgv_viewerReady} until the iframe reports it (which it
 * immediately does, even on the WebGL-missing branch).</p>
 *
 ...
 */
public class ThreeForceGraphViewer extends Browser {

    private static final Logger LOG = Logger.getLogger(ThreeForceGraphViewer.class.getName());
    private static final String DEFAULT_HTML_RESOURCE = "/static/three/three-force-viewer.html";

    private final ThreeForceGraphJsBridge bridge;
    private GraphData currentData;

    private volatile LayoutAlgorithm currentLayout = LayoutAlgorithm.FORCE_3D;
    ...
    public ThreeForceGraphViewer(Composite parent, int style) {
        this(parent, style, DEFAULT_HTML_RESOURCE);
    }

    public ThreeForceGraphViewer(Composite parent, int style, String htmlOrResourcePath) {
        super(parent, style);
        LOG.info("ThreeForceGraphViewer: constructing with htmlOrResourcePath=" + htmlOrResourcePath);
        this.htmlOrResourcePath = htmlOrResourcePath;
        this.bridge = new ThreeForceGraphJsBridge(this);
        LOG.info("ThreeForceGraphViewer: bridge constructed");
        wireBridgeListeners();

        // Apply HTML eagerly. The three-force-viewer.js bridge uses a
        // ResizeObserver (with setInterval polling fallback) to defer
        // initialization until the embedded container has a non-zero
        // size.
        String html = loadClasspathResource(htmlOrResourcePath);
        if (html == null) {
            html = htmlOrResourcePath;
        }
        LOG.info("ThreeForceGraphViewer: html loaded, length=" + (html == null ? 0 : html.length()));
        setText(html);
        LOG.info("ThreeForceGraphViewer: setText(html) called");
    }

    private static String loadClasspathResource(String path) {
        if (path == null || path.isEmpty()) return null;
        try (InputStream in = ThreeForceGraphViewer.class.getResourceAsStream(path)) {
    ...
    public void setGraphData(GraphData data) {
        LOG.info("ThreeForceGraphViewer.setGraphData: nodes=" + (data == null ? 0 : data.getNodes().size())
                + ", edges=" + (data == null ? 0 : data.getRelationships().size())
                + ", bridgeReady=" + (bridge == null ? "null" : bridge.isViewerReady()));
        if (data == null) {
            clear();
            return;
        }
        ...
        runWhenReady(() -> {
            ...
            // tfgv_applyColorPalette(entries, true) — so the Color Palette
            // panel appears automatically for every 3D graph that declares
            // a non-empty palette, without renaming labels to "ClusterN"
            // like the Leiden path does.
     ...
     * {@link GraphEngine#THREE_FORCE_GRAPH} for instances of this class;
     * the vis-network counterpart is {@link GraphViewer}.
     */
    public GraphEngine getEngine() {
        return GraphEngine.THREE_FORCE_GRAPH;
    }
    ...
    /**
     * Push a per-node Leiden cluster color map to the iframe. 3d-force-graph
     * has no stylesheet engine, so each node receives a per-node color
     * update via {@code tfgv_applyLeidenColors}.
     *
     * <p>Pairs with {@link SwitchingViewer#setLeidenClusterColors} which
     * forwards to this method when the active engine is
     * {@code THREE_FORCE_GRAPH}. The button
     * {@code GraphConfigurationDialog.applyLeidenClustering} was a silent
     * no-op under 3D before this method existed.</p>
     */
     ...
     *
     * <p>Pairs with {@link SwitchingViewer#setGraphData} which forwards the
     * palette on every {@code setGraphData(...)} call and on engine
     * round-trips into 3D. The map keys are preserved verbatim as legend
     * row labels (e.g. node id, cluster key, ...) so the panel matches
     * the caller's intent.</p>
     */
    ...
    /**
     * Apply the engine-agnostic per-node effective color map produced by
     * {@link de.tk.dependencyanalyse.rapui.visgraph.internal.NodeColorResolver}.
     *
     * <p>Used by the unified {@link SwitchingViewer#applyNodeColors} so
     * the dialog's Tag-Colors and Leiden-Colors buttons apply identically
     * across Cytoscape, vis-network, sigma, NVL and 3D.</p>
     */
    ...
    /**
     * Push per-node image/icon updates to the iframe. Each entry must
     * carry a {@code {id, ...}} payload.
     *
     * <p>The 3D renderer has no overlayIcon concept (nodes are WebGL
     * spheres), so this method forwards the payload for any downstream
     * consumer and is otherwise a no-op at the JS bridge.</p>
     */
    ...
    public void setContextMenuProvider(ContextMenuProvider provider) {
        if (provider == null) {
            bridge.addContextHandler(new ThreeForceGraphJsBridge.ContextHandler() {
    ...
    private void drainPendingOps() {
        LOG.info("ThreeForceGraphViewer.drainPendingOps: " + pendingOps.size() + " op(s) queued");
        Runnable r;
